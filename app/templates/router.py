...
from app.templates.schemas import (
    TemplateCreateIn, RefreshStatusIn, TemplateOut, TemplateListResponse,
    SendMessageRequest, VariableMappingUpdateRequest,
)
...
async def create_template(
    ...
    if isinstance(result, str) and result.startswith("validation_error:"):
        raise HTTPException(status_code=400, detail=result.replace("validation_error:", ""))
...
# 🚀 NEW: this endpoint didn't exist before — variable mapping choices
# only ever got saved to local SQLite. Now they persist to Mongo too.
@router.patch("/{name}/mapping")
async def update_mapping(
    restaurant_id: str,
    name: str,
    body: VariableMappingUpdateRequest,
    db=Depends(get_database),
):
    result = await service.update_variable_mapping(db, restaurant_id, name, body.variable_mapping)
    if result == "not_found":
